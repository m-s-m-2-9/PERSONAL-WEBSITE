...
(function () {
  ...
  const interval = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      setTimeout(() => { document.getElementById('loading-screen').classList.add('done'); startHeroAnimations(); }, 300);
    }
...
function startHeroAnimations() {
  ['name-part-1','name-part-2','name-part-3'].forEach((id, i) => {
    setTimeout(() => { const el = document.getElementById(id); if (el) el.classList.add('visible'); }, i * 150 + 200);
  });
...
(function () { const saved = localStorage.getItem('msm-theme'); if (saved) setTheme(saved); })();
...
function animateSkillBars(page) { page.querySelectorAll('.skill-item').forEach(item => item.classList.add('animated')); }
...
function unlockSection(inputId, contentId) {
  ...
  if (input.value === CONFIG.MASTER_PASSWORD) {
    content.classList.add('unlocked'); content.style.display = 'block';
    const form = input.closest('.inline-password-form');
    if (form) { form.style.opacity = '0.3'; form.style.pointerEvents = 'none'; }
  } else {
    input.classList.add('error'); input.value = ''; input.placeholder = 'Wrong password ✗';
    setTimeout(() => { input.classList.remove('error'); input.placeholder = 'password'; }, 1500);
  }
}
...
function submitGate() {
  ...
  if (input.value === CONFIG.MASTER_PASSWORD) {
    closeGate(); if (target) doTransition(target);
  } else {
...
const beliefPosts = {
  politics: [
    { date: "April 2026",   title: "xyz Politics post title — REPLACE", body: "xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz xyz. REPLACE." },
...
async function submitContactForm(e) {
  ...
  try {
    ...
    status.textContent = '✓ Message sent. I\'ll be in touch.'; status.className = 'form-status success'; form.reset();
  } catch (err) {
    status.textContent = '✗ Something went wrong. Try again.'; status.className = 'form-status error'; console.error('EmailJS error:', err);
  }
...
function openAlbum(albumId) {
  currentAlbum = albumData[albumId] || { photos: [] }; currentPhotoIndex = 0;
  showPhoto(0); document.getElementById('photo-viewer').classList.add('open');
}
function showPhoto(index) {
  const photos = currentAlbum.photos; if (!photos || photos.length === 0) return;
  currentPhotoIndex = (index + photos.length) % photos.length;
  const photo = photos[currentPhotoIndex];
  const img = document.getElementById('viewer-img');
  img.src = photo.src || `data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='600' height='400'><rect fill='%23222'/><text x='50%25' y='50%25' fill='%23555' text-anchor='middle' dy='.3em' font-family='sans-serif' font-size='16'>Photo placeholder</text></svg>`;
  img.alt = photo.title;
...
function closeViewer() { document.getElementById('photo-viewer').classList.remove('open'); }
...
/* ─── GAME 1: SNAKE — with mobile touch + D-pad ─── */
function renderSnake(container) {
  const isMobile  = window.matchMedia('(pointer: coarse)').matches;
  const canvasSize = isMobile ? Math.min(300, window.innerWidth - 64) : 400;

  container.innerHTML = `
    <h3 style="font-family:var(--ff-display);font-size:1.5rem;color:var(--text);text-align:center;margin-bottom:0.75rem;">Snake</h3>
    <p style="text-align:center;font-size:0.75rem;color:var(--text3);margin-bottom:0.75rem;">${isMobile ? 'Swipe or use D-pad' : 'Arrow keys or WASD'} · Score: <span id="snake-score">0</span></p>
    <canvas id="snake-canvas" width="${canvasSize}" height="${canvasSize}" style="border:1px solid var(--border2);background:var(--bg2);display:block;margin:0 auto;max-width:100%;touch-action:none;"></canvas>
    <p style="text-align:center;font-size:0.75rem;color:var(--text3);margin-top:0.75rem;" id="snake-msg">${isMobile ? 'Swipe canvas or tap D-pad to start' : 'Press any arrow key to start'}</p>
    ${isMobile ? `
    <div class="snake-dpad">
      <div class="snake-dpad-row"><div class="snake-dpad-blank"></div><button class="snake-dpad-btn" id="sn-up">↑</button><div class="snake-dpad-blank"></div></div>
      <div class="snake-dpad-row"><button class="snake-dpad-btn" id="sn-left">←</button><div class="snake-dpad-blank"></div><button class="snake-dpad-btn" id="sn-right">→</button></div>
      <div class="snake-dpad-row"><div class="snake-dpad-blank"></div><button class="snake-dpad-btn" id="sn-down">↓</button><div class="snake-dpad-blank"></div></div>
    </div>` : ''}
  `;

  const canvas = document.getElementById('snake-canvas');
  const ctx    = canvas.getContext('2d');
  const SIZE   = isMobile ? Math.floor(canvasSize / 20) : 20;
  ...
  function draw() {
    ...
    ctx.fillStyle = '#e8d5b7'; ctx.fillRect(food.x * SIZE + 2, food.y * SIZE + 2, SIZE - 4, SIZE - 4);
    snake.forEach((seg, i) => {
    ...
    if (gameOver) {
      ...
      ctx.fillStyle = '#e8d5b7'; ctx.font = `${isMobile ? 16 : 20}px Cormorant Garamond, serif`;
      ctx.textAlign = 'center'; ctx.fillText('Game Over — Score: ' + score, canvas.width / 2, canvas.height / 2);
      ctx.font = `${isMobile ? 11 : 13}px DM Sans, sans-serif`; ctx.fillStyle = '#888';
      ctx.fillText(isMobile ? 'Tap D-pad to restart' : 'Press any key to restart', canvas.width / 2, canvas.height / 2 + 26);
    }
  }
  ...
  function step() {
    ...
    if (head.x === food.x && head.y === food.y) {
      score++; document.getElementById('snake-score').textContent = score; food = spawnFood();
    } else { snake.pop(); }
    draw();
  }
  ...
  function startGame() {
    snake = [{ x: Math.floor(COLS/2), y: Math.floor(ROWS/2) }]; dir = { x: 1, y: 0 }; score = 0;
    document.getElementById('snake-score').textContent = 0;
    food = spawnFood(); gameOver = false; running = true;
    document.getElementById('snake-msg').textContent = '';
    clearInterval(interval); interval = setInterval(step, 120); draw();
  }

  function applyDir(d) {
    if (!running || gameOver) { startGame(); return; }
    if (dir.x !== -d.x || dir.y !== -d.y) dir = d;
  }
  ...
  document.addEventListener('keydown', function snakeKeys(e) {
    if (!document.getElementById('game-modal').classList.contains('open')) { document.removeEventListener('keydown', snakeKeys); return; }
    if (gameOver) { startGame(); return; }
    if (!running) { startGame(); }
    const nd = DIRS[e.key];
    if (nd) { if (dir.x !== -nd.x || dir.y !== -nd.y) dir = nd; e.preventDefault(); }
  });

  /* D-pad buttons (mobile) */
  if (isMobile) {
    const dpadMap = {
      'sn-up':    {x:0,y:-1}, 'sn-down': {x:0,y:1},
      'sn-left':  {x:-1,y:0}, 'sn-right':{x:1,y:0}
    };
    Object.entries(dpadMap).forEach(([id, d]) => {
      const btn = document.getElementById(id);
      if (!btn) return;
      btn.addEventListener('touchstart', (e) => { e.preventDefault(); applyDir(d); }, { passive: false });
    });
...
function renderMemory(container) {
  ...
  container.innerHTML = `
    ...
    <div id="mem-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;max-width:320px;margin:0 auto;"></div>
  `;
  const grid = document.getElementById('mem-grid');
  cards.forEach((emoji, i) => {
    const card = document.createElement('div');
    card.style.cssText = `width:100%;aspect-ratio:1;background:var(--bg3);border:1px solid var(--border2);border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:1.4rem;transition:all 0.3s;user-select:none;touch-action:manipulation;`;
    ...
    card.onclick = () => {
      ...
      flipped.push(i);
      if (flipped.length === 2) {
        locked = true; moves++; document.getElementById('mem-moves').textContent = moves;
...
function render2048(container) {
  ...
  container.innerHTML = `
    ...
    <p style="text-align:center;font-size:0.75rem;color:var(--text3);margin-bottom:1rem;">Score: <span id="g2048-score">0</span> · ${window.matchMedia('(pointer: coarse)').matches ? 'Swipe' : 'Arrow keys'} to slide</p>
    <div id="g2048-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;max-width:320px;margin:0 auto;background:var(--bg3);padding:6px;border-radius:4px;touch-action:none;"></div>
  `;
  ...
  function addRandom() {
    const empty = []; grid.forEach((row,r) => row.forEach((val,c) => { if (!val) empty.push([r,c]); }));
  ...
  let t2x = 0, t2y = 0;
  if (gridEl) {
    gridEl.addEventListener('touchstart', (e) => { e.preventDefault(); t2x = e.touches[0].clientX; t2y = e.touches[0].clientY; }, { passive: false });
    gridEl.addEventListener('touchend', (e) => {
      e.preventDefault();
      const dx = e.changedTouches[0].clientX - t2x;
      const dy = e.changedTouches[0].clientY - t2y;
      const threshold = 30;
      if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > threshold) { move(dx > 0 ? 'right' : 'left'); }
      else if (Math.abs(dy) > threshold) { move(dy > 0 ? 'down' : 'up'); }
    }, { passive: false });
  }
}
...
function renderReaction(container) {
  ...
  container.innerHTML = `
    ...
    <div id="react-box" style="width:min(300px,80vw);height:min(200px,53vw);border-radius:8px;background:var(--bg3);border:1px solid var(--border2);display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;margin:0 auto;transition:background 0.3s ease;touch-action:manipulation;">
      <div id="react-text" style="font-family:var(--ff-display);font-size:1.2rem;color:var(--text2);text-align:center;padding:1rem;"></div>
  ...
  function reactionClick() {
    if (state==='waiting') {
      state='ready'; box.style.background='#c0392b'; text.textContent='Wait...'; sub.textContent=''; clearTimeout(timeout);
      ...
      clearTimeout(timeout); box.style.background='#c0392b'; text.textContent='Too early! Try again.'; setTimeout(reset,1000);
    } else if (state==='go') {
...
function renderWordScramble(container) {
  ...
  function nextWord() {
    const word = words[current%words.length]; let s = scramble(word); while (s===word) s=scramble(word);
    document.getElementById('ws-scramble').textContent=s; document.getElementById('ws-input').value='';
    document.getElementById('ws-input').focus(); document.getElementById('ws-feedback').textContent='';
  ...
  container.innerHTML = `
    ...
    <div style="text-align:center;">
      <div id="ws-scramble" style="font-family:var(--ff-display);font-size:clamp(1.8rem,8vw,3rem);letter-spacing:0.2em;color:var(--accent);margin-bottom:2rem;"></div>
      <div style="display:flex;gap:0;max-width:260px;margin:0 auto;">
        ...
        <button onclick="wsCheck()" style="background:var(--accent);border:1px solid var(--accent);color:var(--bg);padding:0.75rem 1rem;font-family:var(--ff-mono);font-size:0.8rem;touch-action:manipulation;">Go</button>
      </div>
...
const nameClickMessages = [
  ...
  "7 clicks! You found the easter egg. Hello, persistent human. Lean back and calm down.",
];
...
function updateMusicUI() {
  const isPlaying = (bgMusic && !bgMusic.paused && !bgMusic.ended) || (rainSong && !rainSong.paused && !rainSong.ended);
  if (isPlaying) { songBar.classList.remove("muted"); } else { songBar.classList.add("muted"); }
  if (vinylEl) { if (isPlaying) { vinylEl.classList.remove("paused"); } else { vinylEl.classList.add("paused"); } }
  const panelBars = document.getElementById('music-panel-bars');
  if (panelBars) { if (isPlaying) { panelBars.classList.add('playing'); } else { panelBars.classList.remove('playing'); } }
  updateMusicPanelState();
}

function updateMusicPanelState() {
  const nowEl = document.getElementById('music-panel-now'); if (!nowEl) return;
  ...
  if (!isPlaying) { nowEl.textContent = '— paused —'; }
...
songBar.addEventListener("click", () => {
  const isPlaying = (bgMusic && !bgMusic.paused && !bgMusic.ended) || (rainSong && !rainSong.paused && !rainSong.ended);
  if (isPlaying) { bgMusic.pause(); rainSong.pause(); userPaused = true; }
  else {
    userPaused = false;
    if (currentSongId === 'easter' && rainSong.currentTime > 0 && !rainSong.ended) { rainSong.play().catch(() => {}); }
    else { bgMusic.play().catch(() => {}); currentSongId = 'bg'; }
  }
  updateMusicUI();
});
...
rainSong.addEventListener("ended", () => {
  currentSongId = 'bg';
  if (!userPaused) { bgMusic.currentTime = 0; bgMusic.play().catch(() => {}); }
  updateMusicUI();
});

bgMusic.addEventListener("play",  updateMusicUI); bgMusic.addEventListener("pause", updateMusicUI);
rainSong.addEventListener("play", updateMusicUI); rainSong.addEventListener("pause",updateMusicUI);

function tryStartBgMusic() {
  if (musicStarted || userPaused) return;
  bgMusic.play().then(() => { musicStarted = true; currentSongId = 'bg'; updateMusicUI(); }).catch(() => {});
}
...
function toggleMusicPanel() {
  if (!musicPanel) return;
  if (musicPanel.classList.contains('open')) { closeMusicPanel(); } else { openMusicPanel(); }
}

if (vinylEl) { vinylEl.addEventListener("click", () => { toggleMusicPanel(); }); }

function showVinyl() {
  if (!vinylEl) return; vinylEl.classList.remove("hidden"); easterUnlocked = true;
}

function hideVinyl() {
  if (!vinylEl) return;
  closeMusicPanel();
  setTimeout(() => { vinylEl.classList.add("hidden"); easterUnlocked = false; }, 200);
}

function playFromPanel(songId) {
  bgMusic.pause(); rainSong.pause(); userPaused = false; currentSongId = songId;
  if (songId === 'bg') { bgMusic.currentTime = 0; bgMusic.play().catch(() => {}); }
  else if (songId === 'easter') { rainSong.currentTime = 0; rainSong.play().catch(() => {}); }
  musicStarted = true; updateMusicUI();
}

function nameclickHandler() {
  nameClicks++;
  const hint = document.getElementById("name-click-hint");
  if (nameClicks >= 5) { hint.textContent = nameClickMessages[Math.min(nameClicks, 7)]; hint.classList.add("show"); }
  if (nameClicks >= 7) {
    if (easterUnlocked) {
      hint.textContent = "Easter egg already enabled! Check the bottom-left corner 🎵";
      hint.classList.add("show");
      setTimeout(() => { nameClicks = 0; hint.classList.remove("show"); }, 3000);
      return;
    }
    showVinyl(); bgMusic.pause(); currentSongId = 'easter'; userPaused = false;
    rainSong.currentTime = 0; rainSong.play().catch(() => {}); musicStarted = true;
    updateMusicUI();
    setTimeout(() => { nameClicks = 0; hint.classList.remove("show"); }, 4000);
  }
}
